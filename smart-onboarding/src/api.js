...
export async function verifyPlate(plate) {
  ...
  const hit = VEHICLES.find(v => v.plate === plateUpper);
  if (hit) return { status: 'existing', plate: hit.plate };
  return { status: 'new', plate: plateUpper, message: 'Plate format is valid' };
}
...
/**
 * Mock: confirm ownership by last 4 NRIC digits using VEHICLES dataset
 */
